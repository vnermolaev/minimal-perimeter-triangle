...
import { Paper } from './demo/drawing';
...
function dbg( f: TraceBit) {
    if (dev) {
        f();
    }
}

function setup(points: Vec2[]): void {
    paper.clear();
    paper.points(points, 3, 'red');
    paper.points([points[0], points[points.length-1] ], 5, 'maroon');
}
...
function lineTangentToHull(line: Line, points: Vec2[]): {holds: boolean, side: number} {
    ...
    dbg(()=> {
        setup(points);
        paper.lines([line.start.plus(line.delta.times(10)), line.start.minus(line.delta.times(10))]);
    });
    for (let i: number = 0; i<points.length; i++) {
        let side0: number = line.pointOnSide( points[i], 10**-5 );
        // point is on the line
        if ( side0 === 0 ) { continue; }
        //point doesnt on a line
        if ( side === 0 ) {
        ...
        if ( side0 !== side ) {
            holds = false;
            dbg(()=> { paper.points([points[i]], 5, 'yellow'); });
            break;
        }
    }
...
function findInitialArm(dWedge: Inscribe.Wedge, endVertex: number, points: Vec2[]): Line {
    ...
    dbg( ()=> {
        setup(points);
        paper.lines([dWedge.left_arm.start, dWedge.left_arm.end]);
        paper.lines([dWedge.right_arm.start, dWedge.right_arm.end]);
    });
    ...
    do {
        ...
        if (!edgePossibility) {
            console.log('exhausted during initial wedge estimation');
        }
        ...
        if ( !Qfound && edgePossibility &&
            ...
            dbg(()=> {
                ...
                paper.lines([dWedge.left_arm.start, dWedge.left_arm.end]);
                paper.lines([dWedge.right_arm.start, dWedge.right_arm.end]);
                paper.lines([edge.start, edge.end], 'green');
            });
            ...
            if (circlesWedgeLine !== null) {
                dbg(()=> {
                    circlesWedgeLine.forEach( ( circleWedgeLine: {circle: Inscribe.Circle, tangentParameter: number} )=> {
                        paper.circle(circleWedgeLine.circle.centre, circleWedgeLine.circle.r);
                    });
                });
                ...
                if ( 0 < tangentParameter && tangentParameter < 1 ) {
                    ...
                    dbg( ()=> { paper.lines([arm.start, arm.end.plus(arm.delta)], 'red'); });
                }
            }
        }
        ...
        if ( !Qfound &&
            ...
            dbg(()=> {
                ...
                paper.lines([dWedge.left_arm.start, dWedge.left_arm.end]);
                paper.lines([dWedge.right_arm.start, dWedge.right_arm.end]);
                paper.points([point], 5, 'green');
            });

            let circlesWedgePoint = Inscribe.DegenWedgePoint( dWedge, point );

            dbg(()=> {
                circlesWedgePoint.forEach( ( circleWedgePoint: {circle: Inscribe.Circle, tangentLine: Line} )=> {
                    paper.circle(circleWedgePoint.circle.centre, circleWedgePoint.circle.r);
                });
            });

            if ( circlesWedgePoint !== null /*&&
                 // if length is 1, it means that point lies on one of the arms and therefore no triangle can be formed
                 circlesWedgePoint.length > 1*/
                ) {
...
function findThirdSide(wedge: Inscribe.Wedge, startVertex: number, endVertex: number, points: Vec2[]): Line {
    ...
    dbg( ()=> {
        setup(points);
        paper.lines([wedge.left_arm.start, wedge.left_arm.end]);
        paper.lines([wedge.right_arm.start, wedge.right_arm.end]);
    });
    ...
    do {
        ...
        if (!edgePossibility) {
            console.log('exhausted during wedge flipping');
        }
        ...
        if ( !PQfound && edgePossibility &&
            ...
            if ( circleWedgeLine !== null ) {
                ...
                dbg(()=> { paper.circle(circleWedgeLine.circle.centre, circleWedgeLine.circle.r); });
                ...
                if ( 0 < circleWedgeLine.tangentParameter && circleWedgeLine.tangentParameter < 1 ) {
                    ...
                    dbg( ()=> { paper.lines([side.start, side.end.plus(side.delta)], 'red'); });
                }
            }
        }
        ...
        if ( !PQfound &&
            ...
            if ( circleWedgePoint !== null ) {
                ...
                dbg(()=> { paper.circle(circleWedgePoint.circle.centre, circleWedgePoint.circle.r); });
...
function triangleWithBase(points: Vec2[]): {A: Vec2, B: Vec2, C: Vec2} {
    ...
    dbg(()=> { paper.lines([BC.start, BC.end, BC.end.plus( BC.delta.over(10) )]); });
    ...
    // by default baseParallel points in the same direction as BC

    dbg(()=> {
        paper.lines([baseParallel.start, baseParallel.start.plus(baseParallel.delta)]);
    });
...
export function minimalPerimeterTriangle(convexHull: {x: number, y: number}[], output: Paper, stopIter: number = -1):
    ...
    while ( rotations < points.length ) {
        ...
        dev =  rotations === stopIter;
        if (dev) {
            console.log(`verbose mode for ${rotations}`);
        }
        ...
        if ( triangle === null ) {
            console.log(`failed at ${rotations}\n`);
            rotations++;
            continue;
        }
        ...
        dbg(()=> { setup(points); paper.lines([A1, B1, C1, A1], 'LightSteelBlue'); });
        ...
        if ( A === undefined ) {
            ...
            if (perimeter1 < perimeter) {
                ...
                dbg(()=> { setup(points); paper.lines([A, B, C, A], 'Green'); });
            }
        }
